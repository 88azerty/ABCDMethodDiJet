...
void LLGAnalysis::SetupZJetsTestRegion() {
    ...
    _cutFlow.insert(pair<string,int>("3_HasPVJet", 0) );
    _cutFlow.insert(pair<string,int>("4_HasSV20", 0) );
    _cutFlow.insert(pair<string,int>("5_DiJetMass", 0 ) );
    _cutFlow.insert(pair<string,int>("6_2TightMuons", 0) );
    _cutFlow.insert(pair<string,int>("7_ZWindow", 0 ) );
    _cutFlow.insert(pair<string,int>("8_PseudoMET", 0 ) );
...
void LLGAnalysis::ZJetsTestRegionSelection() {
    ...
    if( PVWithJet.size() >= 1 ) {
        _cutFlow.at("3_HasPVJet") += 1;
        
        if( SVWith2Jets.size() > 0 ) {
            _cutFlow.at("4_HasSV20") += 1;
            ...
            }
            {
                ...
                _cutFlow.at("5_DiJetMass") += 1;
                if( tightMuons.size() != 2 ) return; 
                _cutFlow.at("6_2TightMuons") += 1;
                double mLL = 0.;
                int mID1 = tightMuons.at(0);
                int mID2 = tightMuons.at(1);
                double pxmm = muon_px->at(mID1) + muon_px->at(mID2);
                double pymm = muon_py->at(mID1) + muon_py->at(mID2);
                double pzmm = muon_pz->at(mID1) + muon_pz->at(mID2);
                double e1 = sqrt( muon_px->at(mID1)*muon_px->at(mID1) + muon_py->at(mID1)*muon_py->at(mID1) + muon_pz->at(mID1)*muon_pz->at(mID1) );
                double e2 = sqrt( muon_px->at(mID2)*muon_px->at(mID2) + muon_py->at(mID2)*muon_py->at(mID2) + muon_pz->at(mID2)*muon_pz->at(mID2) );
                double emm = e1+e2;
                TLorentzVector vZ;
                vZ.SetPxPyPzE( pxmm, pymm, pzmm, emm );
                bool isZCandidate = false;
                if( fabs(vZ.M() - 91.187) < 10. ) isZCandidate = true;
                if( !isZCandidate ) return;
                _cutFlow.at("7_ZWindow") += 1;
                
                double pseudoMET_x = met_x->at(SYSMET) + muon_px->at(tightMuons.at(0)) + muon_px->at(tightMuons.at(1));
                double pseudoMET_y = met_y->at(SYSMET) + muon_py->at(tightMuons.at(0)) + muon_py->at(tightMuons.at(1));
                double pseudoMET = sqrt( pseudoMET_x*pseudoMET_x + pseudoMET_y*pseudoMET_y );
                if( pseudoMET > MET_CUT ) {
                  _cutFlow.at("8_PseudoMET") += 1;
                }
